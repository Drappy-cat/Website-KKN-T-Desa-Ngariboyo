...
def convert_to_svg(src_path: Path, dest_path: Path = None):
  ...
  <image width="{width}" height="{height}" href="data:{mime_type};base64,{encoded}"/>
</svg>
"""
...
def convert_directory(dir_path: Path, recursive: bool = True):
    ...
    print(f"\nSelesai: {success_count}/{len(unique_files)} gambar berhasil dikonversi ke SVG dengan warna 100% presisi.")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # Default: scan folder src/assets dan src/imports
